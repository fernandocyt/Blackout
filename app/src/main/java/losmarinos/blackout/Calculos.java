...
public class Calculos {
    ...
    public static List<valorEmpresa> cantidadCortesTotal(Constantes.SERVICIO servicio)
    {
    ...
    public static List<valorEmpresa> porcentajeCortesProgramados(Constantes.SERVICIO servicio)
    {
        List<Corte> cortes = Global.cortes;
        List<Empresa> empresas = Global.empresas;
        int total_cortes_servicio = 0;
    ...
    public static List<valorEmpresa> calificaciones(Constantes.SERVICIO servicio)
    {
        List<Empresa> empresas = Global.empresas;
        List<valorEmpresa> stat_empresas = new ArrayList<>();
        ...
        for(int i = 0; i < empresas.size(); i++){
            if(servicio == null || empresas.get(i).getTipoServicio() == servicio) {
                valorEmpresa nuevo_stat = new valorEmpresa();
                nuevo_stat.id_empresa = empresas.get(i).getSubId();
                nuevo_stat.valor = calificacionEmpresa(empresas.get(i));
                nuevo_stat.unidad = "ptos.";
                stat_empresas.add(nuevo_stat);
            }
        }
        return stat_empresas;
    ...
    public static List<valorEmpresa> ocurrenciasBarrio(Context contexto, Constantes.SERVICIO servicio)
    {
        List<Empresa> empresas = Global.empresas;
        List<valorEmpresa> stat_empresas = new ArrayList<>();
        ...
        for(int i = 0; i < empresas.size(); i++){
            if(servicio == null || empresas.get(i).getTipoServicio() == servicio) {
                valorEmpresa nuevo_stat = new valorEmpresa();
                nuevo_stat.id_empresa = empresas.get(i).getSubId();
                nuevo_stat.valor = 99999; // esto es para que despues se muestre esolo la unidad

                List<Corte> cortes_empresa = Global.encontrarCortesPorEmpresa(empresas.get(i).getSubId());
                nuevo_stat.unidad = Calculos.barrioConMasOcurrencias(contexto, cortes_empresa);
                stat_empresas.add(nuevo_stat);
            }
        }
        return stat_empresas;
    ...
    public static String barrioConMasOcurrencias(Context contexto, List<Corte> cortes) {
        Geocoder geocoder = new Geocoder(contexto, Locale.getDefault());

        String barrio_elegido = "no hay datos";
        int max_ocurrencias = 0;
        for (int j = 0; j < cortes.size(); j++) {
            int ocurrencias = 0;

            for (int k = 0; k < cortes.size(); k++) {
                if(hayInterseccion(cortes.get(j).getUbicacion(), 1000, cortes.get(k).getUbicacion(), 1000)){
                    ocurrencias++;
                }
            }
